[English](https://github.com/Somnia1337/Art-Wallpaper/blob/main/README.md) | 中文

# [名画映像](https://github.com/Somnia1337/Art-Wallpaper)

我的 **名画二创** 作品，提供 **8K 超高清（7680 \* 4320）** 分辨率。

<div align=center>
  <img src="https://github.com/Somnia1337/Art-Wallpaper/blob/main/preview.png?raw=true" width="1920px">
</div>

# 作品列表

收藏的名画作品：

| Title                                           | Artist                            | Year      |
| ----------------------------------------------- | --------------------------------- | --------- |
| A Coastal Landscape in the South of France      | 尤金·罗伯特 (Eugene Robert)       | 未知      |
| A Dream                                         | 巴勃罗·毕加索 (Pablo Picasso)     | 1932      |
| A Sunday on La Grande Jatte                     | 乔治·修拉 (Georges Seurat)        | 1884-1886 |
| Green Wheat Fields                              | 文森特·梵高 (Vincent van Gogh)    | 1890      |
| Impression, Sunrise                             | 克劳德·莫奈 (Claude Monet)        | 1872      |
| Marine                                          | 查尔斯·阿塔米安 (Charles Atamian) | 未知      |
| Self-Portrait with Grey Felt Hat                | 文森特·梵高 (Vincent van Gogh)    | 1887      |
| Starry Night Over the Rhone                     | 文森特·梵高 (Vincent van Gogh)    | 1888      |
| The Great Wave off Kanagawa                     | 葛饰北斋 (Katsushika Hokusai)     | ~1831     |
| The Persistence of Memory                       | 萨尔瓦多·达利 (Salvador Dalí)     | 1931      |
| The Scream                                      | 爱德华·蒙克 (Edvard Munch)        | 1893      |
| The Son of Man                                  | 雷内·马格利特 (René Magritte)     | 1964      |
| The Starry Night                                | 文森特·梵高 (Vincent van Gogh)    | 1889      |
| The Treachery of Images                         | 雷内·马格利特 (René Magritte)     | 1929      |
| Wheat Field with Cypresses                      | 文森特·梵高 (Vincent van Gogh)    | 1889      |
| Woman with a Parasol - Madame Monet and Her Son | 克劳德·莫奈 (Claude Monet)        | 1875      |

# 二创历程

**画作**：从海量网络资源中挑选。

**背景色**：用取色器从画作图像中反复选取（每件作品约 10-30 次取色），具有一定的随机性（这也是艺术创作所必需的）。

**制作**：一个简单的网页文件（`template.html`）。

**截取**：用浏览器的开发者工具（具体方法略）。
